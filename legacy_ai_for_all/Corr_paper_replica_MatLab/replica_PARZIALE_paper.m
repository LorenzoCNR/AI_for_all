...
% 2) ho allineato i dati camionando come lo fanno loro
...
for i = 1:wpm
    ...
    while start_idx + blocco_samples - 1 <= idx_max
        end_idx = start_idx + blocco_samples - 1;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%% RIDGE REGRESSION %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% RIDGE REGRESSION %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% la ridge regression viene fatta per ogni blocco e per ogni canale/elettrodo
% devo quindi estrarre dalla struttura i vettori (40*1) per movimento per
% elettrodo...ora mi trovo di fronte a un problema diverso rispetto al
% paper: gli embedding che generano loro come detto sopra, sono statici e
% quidni sono sempre gli stessi per ogni blocco/lag e quello che fanno è
% fare una media temporale sui dati neurali per blocco e per neurone e costruire un
% numero di vettori Y di dimensione numero di parole *1 e fare un nuemro di
% regressioni pari a numero di blocchi * numero di neruoni usando sempre la
% stessa X. In pratica:
% X ha dimensione (#parole*spazio di proiezione)
% y ha dimensione (#parole*1...ripetuto un numero di volte pari al numero
% di lag(o blocchi) per numero di  neuroni

% Noi abbiamo una struttura di dati neurali analoga (tante celle quianti
% sono i movimenti e in ogni cella ogni blocco\lag ha dimensione
% t_lag*neuroni; tuttavia le X (embeddding= contengono anche essi una
% dimensione temporale (che nel paper non hanno) e sono ricostrubili come 
% le X in tante celle quanti sono i movimenti (64) ognuna contenente una
% matrice t_lag*dimensione output (3). Ora per non perdere questa info
% temporale, invece che fare una media sul tempo posso mettere i dati in
% formato long in modo da avere per ogni neurone per ogni lag non una
% matrice t_lag*#movimenti bensi un vettore (t_lag*#movimenti)*1.
% Analogamente procedo con gli embedding in cui avrò per ogni lag e per
% ogni neurone una matrice X (t_lag*#movimenti)*dimensione output
...
n_blocchi = size(dati_listener{1}, 1); 
n_channels % Numero di elettrodi per blocco
blocco_samples % lunghezza blocco
...
% quidni ora abbiamo due strutture Y che contengono  61*64 elementi
...
% correlazioni  
coefs_s_cv = cell(numBlocchi, n_channels);
y_hat_s_cv = cell(numBlocchi, n_channels);
corrs_s_cv = zeros(numBlocchi, n_channels, K);
%listener
coefs_l_cv = cell(numBlocchi, n_channels);
y_hat_l_cv = cell(numBlocchi, n_channels);
corrs_l_cv = zeros(numBlocchi, n_channels, K);
...
% speaker-listener
corrs_inter_s_l = zeros(numBlocchi, n_channels, K);
% listener-speaker
corrs_inter_l_s = zeros(numBlocchi, n_channels, K);
for b = 1:numBlocchi
    for e = 1:n_channels
