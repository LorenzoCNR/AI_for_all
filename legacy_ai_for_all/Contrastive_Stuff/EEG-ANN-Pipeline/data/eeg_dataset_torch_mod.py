        ...
        # Se le label sono array (es. coordinate bidimensionali o più dimensioni)
            ...
            for label_name in self.label_names:
                ...
                if isinstance(labels_i[0], (list, np.ndarray)):
                    labels_i_array = np.array(labels_i)
                    if labels_i_array.ndim in (1, 2):  # Bidimensionale o unidimensionale
                        labels[label_name] = torch.FloatTensor(labels_i_array)
                    elif labels_i_array.ndim > 2:  # Multidimensionale
                        labels[label_name] = torch.FloatTensor(labels_i_array)
                    else:
                        raise ValueError(f"Label '{label_name}' ha una forma non gestita: {labels_i_array.shape}")
                # Gestione di interi (scalari)
                elif isinstance(labels_i[0], (int, np.integer)):
                    labels[label_name] = torch.LongTensor(labels_i)
                else:
                    raise ValueError(f"Formato delle label '{label_name}' non supportato: {type(labels_i[0])}")

        # Salvo le label nella classe
        self.labels = labels

        # Salvo il dizionario di conversione, se esiste
