...
### DIRECTION distance dealt just a qualitative one 
# (couldbe imporved considering that 1 differs 5 more than it differs from 2)
'''
...
'''
Data Structure:
   - X (neural data) is a matrix T(ime)xchns
   - y_ are the associated labels, which can be:
    Discrete or continuous.
    Either matrices or vectors with dimensions T×nT×n, meaning:
        Labels exist for each time point.
        nn is the number of label dimensions.
...
# identify trials (l'info deve essere nel database o va inferita in qualche modo
# - tipo dove y_dir cambia...con la cautela che potrebbe non cambiare da un trial 
# all'altro)...o magari sapendo come nelle scimmie cebra la lunghezzA DEL TRIAL
# è costante e quindi fai la divisione sulla lunghezza totale
# se già sai quanti sono i trials è più semplice

#trial_id=data['m1_active_trial']
#trial_id=trial_id.flatten()
#n_trials_=127
len_y=len(y_dir)
#change_idx = np.where(np.diff(trial_id) != 0)[0] + 1
# questo vettore deve avere un numero di elementi pari al numero di trial+1
# e partire da zero per arrivare alla lunghezza T dei dati
c_t= np.linspace(0, len_y, n_trials+1, dtype=int)
#n_trials=len(c_t)-1
#c_t=np.concatenate([[0], change_idx,[len_y]])

### check trial length (useful for grpahics)
trial_len=np.diff(c_t)
if np.var(trial_len)==0:
    constant_len=True
    trial_len=np.unique(trial_len)
else:
    constant_len=False


### trial_ids is a vector telling us the trial at time t
#### questo va cambiato sulla base dell'id cfr codice e dati mirco
#trial_ids=y_dir
...
#### Either first (0) or second dimension (1)
chns = X.shape[1]
print(chns)
...
### distanza periodica sulla base del tipo di movimento (mano che va in 8 direzioni)
