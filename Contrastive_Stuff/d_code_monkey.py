...
def create_windows(dataset, window, shift, chns):
    ...
    for trial in dataset.trials:
        ...
        position_label = trial.eeg_signals[chns:chns+2, :]
        direction_label = trial.eeg_signals[chns + 2, :]
        ...
        while ind_center + window // 2 < len(times):
            ...
            label_full = {'Position': position_label[:, ind_center], 'Direction': int(
                direction_label[ind_center])}
...
#path =default_input_dir / f"{name}.jl"
    ...
    #data['ephys']=ephys_EDF1
    
    #jl.dump(data, 'dati_aggiornati')
    
   
    chns = data['spikes_active'].shape[1]
...
# print(chns)
    ...
    ### trial_ids is a vector telling us the trial a time t
    ### c_t is a counter of trials
    trial_ids=data['active_target']
    ...
    # spikes = data['spikes']
    #trials=np.linspace(c_t[0] / fs, c_t[1]/fs,c_t[i+1] - c_t[i])
    #spikes_and_label[c_t[0]]
    # prepare data
    data['active_target']=data['active_target'].reshape(-1,1)
