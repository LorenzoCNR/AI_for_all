function   data_struct = hc11ArrangeTrials(data_neur,data_behav)
% function data_struct = hc11ArrangeTrials(data_neur,data_behav)

% data_behav = T x 3. (times x info) 
% data_behav(:,1) distance from the center
% data_behav(:,2) going left
% data_behav(:,3) going right

% construct data_behav(:,4) - trial identify: one trial from leftward to righward point
data_behav(:,4)=1;
k=1;
...
for iTrial = 1:nTrials
    ...
    data_struct(iTrial).(OutField)  = data_neur(idx_spikes, 1:end-1)';
    data_struct(iTrial).labels      = data_behav(idx_labels, 1:end-1)';
    n_sp                            = size(data_struct(iTrial).spikes,2);
    data_struct(iTrial).([xfld OutField])= linspace(0,n_sp*n_s,n_sp);

    % nTimes                          = length(data_trials(iTrial).([xfld OutField])); 
% for iTrial=1:nTrials
