...
def plot_cebra(transformed_hdf_path):
     with h5py.File(transformed_hdf_path, 'r') as hdf:
        
        group = hdf['TransformedData']
        emb = group['transformed_data'][:] 
        label = group['labels'][:] 
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Too few arguments. Usage: python script.py transformed_hdf_path rat_data_hdf_path")
    else:
        transformed_hdf_path = sys.argv[1]
        #rat_data_hdf_path = sys.argv[2]
        plot_cebra(transformed_hdf_path)