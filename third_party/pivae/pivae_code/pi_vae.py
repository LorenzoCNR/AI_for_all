...
def custom_data_generator(x_all, u_all):
    while True:
        for x, u in zip(x_all, u_all):
            # Assicurati che x e u siano bidimensionali e del tipo di dati corretto
            x = x.astype('float32')  # Converti in float32 se necessario
            u = u.astype('float32')  # Converti in float32 se necessario

            # Ristruttura i dati se sono tridimensionali (es. [batch_size, height, width] a [batch_size, features])
            # x = x.reshape((x.shape[0], -1))  # Solo se necessario
            # u = u.reshape((u.shape[0], -1))  # Solo se necessario
