...
intorno_sec=4;
word_onsets = sort(randi([intorno/2*fs length(tt)-intorno_sec/2*fs], 1, wpm));
...
n_blocchi = size(blocchi_dati{1}, 1); 
...
for b = 1:numBlocchi
    for e = 1:n_channels
        ...
        for p = 1:wpm
            ...
            Y_s{b,e} = [Y_s{b,e}; mean(blocchi_dati{p}{b}(:, e))];
...
% speaker
coefs_s = cell(numBlocchi, n_channels);
y_hat_s = cell(numBlocchi, n_channels);
% listener
coefs_l = cell(numBlocchi, n_channels);
y_hat_l = cell(numBlocchi, n_channels);
